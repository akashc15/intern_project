const data = {
  ...
  function update(){
    let assets = data.tasks[0].assets
     ...
     if(i!=0)
     {
  document.querySelector(`.tech${i}`).innerHTML = fdata.asset_title
  document.querySelector(`.des${i}`).innerHTML = 'Description'
  document.querySelector(`.d${i}`).innerHTML = ': '
  document.querySelector(`.span${i}`).innerHTML= fdata.asset_description
    }
    else{
        document.querySelector('.tech').innerHTML = fdata.asset_title
        document.querySelector('.des').innerHTML = 'Description'
        document.querySelector('.d').innerHTML = ': '
        document.querySelector('.span').innerHTML= fdata.asset_description
    }
} 

  const head = data.tasks[0].task_title
  document.querySelector('.head').innerHTML = head
  const sub = data.tasks[0].task_description
